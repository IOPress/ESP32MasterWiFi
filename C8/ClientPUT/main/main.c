...
Page initPage(char req[], char port[], char url[], char path[], char buffer[], int len, char *senddata)
{
    Page page;
    page.req = req;
    page.url = url;
...
void asyncgetPage(void *arg)
{
    ...
    char headerTemplate[] = "%s %s HTTP/1.1\r\n"
                            ...
                            "Connection: close\r\n"
                            "Content-length: %d\r\n\r\n%s";
    int len = snprintf(NULL, 0, headerTemplate, page->req, page->path, page->url, page->port, strlen(page->senddata), page->senddata);
    char *requestData = malloc(len + 1);
    snprintf(requestData, len + 1, headerTemplate, page->req, page->path, page->url, page->port, strlen(page->senddata), page->senddata);
...
void app_main(void)
{
    ...
    char path[] = "/";
    Page pagetemp1 = initPage("PUT", "80", url, path, buffer1, 4000, "20.34");
    doPage(&pagetemp1);
